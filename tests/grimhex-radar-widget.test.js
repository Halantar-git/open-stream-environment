...
const WidgetGrimHexRadar = require("../overlay/widgets/grimhex-radar-widget");
...
describe("WidgetGrimHexRadar", () => {
  test("запускает цикл и рисует только для темы grimhex", () => {
    const parent = makeEl("div");
    const w = new WidgetGrimHexRadar({ ...radarItem(), renderType: "canvas" }, makeContext("grimhex"));
  ...
  test("не запускает цикл на чужой теме (уровень виджета)", () => {
    const parent = makeEl("div");
    const w = new WidgetGrimHexRadar({ ...radarItem(), renderType: "canvas" }, makeContext("nebula"));
  ...
  test("донат порождает контакт-корабль, остальные алерты — нет", () => {
    ...
    const parent = makeEl("div");
    const w = new WidgetGrimHexRadar({ ...radarItem(), renderType: "canvas" }, ctx);
    w.mount(parent);
  ...
  test("применяет настройку прозрачности", () => {
    const parent = makeEl("div");
    const w = new WidgetGrimHexRadar({ ...radarItem(), config: { opacity: 25 }, renderType: "canvas" }, makeContext("grimhex"));
