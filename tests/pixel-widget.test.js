...
const WidgetPixelCube = require("../overlay/widgets/pixel-cube-widget");
...
function makeCtx2D() {
  const ctx = {};
  ["setTransform", "clearRect", "save", "restore", "translate", "rotate", "transform", "scale", "beginPath", "moveTo", "lineTo", "bezierCurveTo", "fillRect", "roundRect", "arc", "closePath", "stroke", "fill", "drawImage", "strokeRect"].forEach(
    (m) => (ctx[m] = jest.fn())
  );
...
function cubeItem(id = "c") {
  return { id, type: "pixel-cube", x: 0, y: 0, w: 20, h: 10, z: 0, visible: true, config: {} };
}
...
describe("WidgetPixelCube", () => {
  test("запускает цикл и рисует только для темы pixel", () => {
    const parent = makeEl("div");
    const w = new WidgetPixelCube({ ...cubeItem(), renderType: "canvas" }, makeContext("pixel"));
    ...
    raf.flush(1000);
    expect(w.element._ctx.stroke).toHaveBeenCalled();
  ...
  test("не запускает цикл на чужой теме (уровень виджета)", () => {
    const parent = makeEl("div");
    const w = new WidgetPixelCube({ ...cubeItem(), renderType: "canvas" }, makeContext("nuclear"));
  ...
  test("pop активируется по событию чата/доната", () => {
    const ctx = makeContext("pixel");
    const parent = makeEl("div");
    const w = new WidgetPixelCube({ ...cubeItem(), renderType: "canvas" }, ctx);
    w.mount(parent);
...
describe("WidgetManager — изоляция PixelCube", () => {
  test("не создаёт pixel-cube, пока тема не pixel", () => {
    const root = makeEl("div");
    const context = { bus: new EventBus(), EVENT_TYPES, theme: "nebula" };
    const mgr = new WidgetManager(root, {
      shouldMount: (item) => item.type !== "pixel-cube" || context.theme === "pixel",
      resolveRenderType: (item) => (item.type === "pixel-cube" && context.theme === "pixel" ? "canvas" : "2d"),
      context,
    });
    mgr.register("pixel-cube", WidgetPixelCube);

    mgr.syncLayout([cubeItem()]);
    expect(mgr.size).toBe(0);

    context.theme = "pixel";
    mgr.syncLayout([cubeItem()]);
    expect(mgr.size).toBe(1);
    expect(mgr.get("c").theme).toBe("pixel");
