...
const WidgetNuclearHoloAlert = require("../overlay/widgets/nuclear-holo-alert-widget");
...
function item(overrides = {}) {
  return { id: "h", type: "nuclear-holo-alert", x: 0, y: 0, w: 26, h: 14, z: 0, visible: true, config: {}, renderType: "2d", ...overrides };
}
...
describe("WidgetNuclearHoloAlert", () => {
  test("монтируется в режиме ожидания и запускает цикл только для nuclear", () => {
    const parent = makeEl("div");
    const w = new WidgetNuclearHoloAlert(item(), makeContext("nuclear"));
  ...
  test("не инициализируется на чужой теме (уровень виджета)", () => {
    const parent = makeEl("div");
    const w = new WidgetNuclearHoloAlert(item(), makeContext("nebula"));
  ...
  test("показывает алерт, включает цикл и спавнит частицы", () => {
    ...
    const parent = makeEl("div");
    const w = new WidgetNuclearHoloAlert(item(), ctx);
    w.mount(parent);
...
describe("WidgetManager — изоляция nuclear-holo-alert", () => {
  test("не создаёт nuclear-holo-alert, пока тема не nuclear", () => {
    ...
    const mgr = new WidgetManager(root, {
      shouldMount: (it) => (it.type !== "nuclear-holo-alert" ? true : context.theme === "nuclear"),
      resolveRenderType: () => "2d",
      context,
    });
    mgr.register("nuclear-holo-alert", WidgetNuclearHoloAlert);
