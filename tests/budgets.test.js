...
/*
  ...
  Проверяются величины, которые легко испортить незаметно:
    ...
    * время записи снапшота — берём из собственной телеметрии стора; если
      запись начнёт занимать сотни миллисекунд, это будет видно и в логе, и здесь;
...
describe("бюджеты производительности", () => {
  ...
  test("запись снапшота укладывается в бюджет (по собственной телеметрии стора)", async () => {
    for (let i = 0; i < 5; i++) db.saveWidgets([...db.getWidgets()]);
    await db.flush();

    const stats = db.getWriteStats().database.total;
    expect(stats.writes).toBeGreaterThan(0);
    expect(stats.maxMs).toBeLessThan(BUDGET.writeMaxMs);
  });
