...
const WidgetGrimHex = require("../overlay/widgets/grimhex-widget");
...
function grimhexItem(id = "g") {
  return { id, type: "grimhex", x: 0, y: 0, w: 20, h: 10, z: 0, visible: true, config: {} };
}
...
describe("WidgetGrimHex", () => {
  ...
  test("не запускает цикл на чужой теме (уровень виджета)", () => {
    const parent = makeEl("div");
    const w = new WidgetGrimHex({ ...grimhexItem(), renderType: "canvas" }, makeContext("nebula"));
  ...
  test("glitch активируется по событию чата/доната", () => {
    ...
    const parent = makeEl("div");
    const w = new WidgetGrimHex({ ...grimhexItem(), renderType: "canvas" }, ctx);
    w.mount(parent);
...
describe("WidgetManager — изоляция Grim HEX", () => {
  test("не создаёт grimhex виджет, пока тема не grimhex", () => {
    const root = makeEl("div");
    const context = { bus: new EventBus(), EVENT_TYPES, theme: "nebula" };
    const mgr = new WidgetManager(root, {
      shouldMount: (item) => item.type !== "grimhex" || context.theme === "grimhex",
      resolveRenderType: (item) => (item.type === "grimhex" && context.theme === "grimhex" ? "canvas" : "2d"),
      context,
    });
    mgr.register("grimhex", WidgetGrimHex);

    mgr.syncLayout([grimhexItem()]);
    expect(mgr.size).toBe(0); // пропущен

    context.theme = "grimhex";
    mgr.syncLayout([grimhexItem()]);
    expect(mgr.size).toBe(1);
    expect(mgr.get("g").theme).toBe("grimhex");
    expect(mgr.get("g").renderType).toBe("canvas");
