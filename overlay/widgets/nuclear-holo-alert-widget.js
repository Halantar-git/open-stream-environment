...
/*
  WidgetNuclearHoloAlert — radioactive alert terminal for the Nuclear theme.
  ...
  + border + radius + shadow + CRT scanlines + corner brackets). The canvas
  draws a slowly rotating trefoil radiation badge; on each new alert the badge
...
(function (root, factory) {
  ...
  const WidgetNuclearHoloAlert = factory(BaseWidget);

  if (typeof module !== "undefined" && module.exports) {
    module.exports = WidgetNuclearHoloAlert;
  } else {
    root.OSEWidgets = root.OSEWidgets || {};
    root.OSEWidgets.WidgetNuclearHoloAlert = WidgetNuclearHoloAlert;
  }
  ...
  class WidgetNuclearHoloAlert extends BaseWidget {
    ...
    onMount() {
      ...
      this.canvas.className = "nuclear-holo-alert__canvas";
      ...
      this.contentEl.className = "nuclear-holo-alert__content";
      ...
      this.element.classList.add("nuclear-holo-alert-surface");
      this._applySurface();
    ...
    // radius, drop shadow/glow, CRT scanline texture + corner brackets.
    _applySurface() {
      ...
      // CRT decoration ([data-decoration="nuclear"]) can layer on top.
    ...
    kindColor(kind) {
      ...
      switch (kind) {
        ...
        case "wheel_start":
        case "wheel_winner":
          return (read && read("--md-secondary")) || GREEN;
        case "cheer":
        case "donation":
          return (read && read("--md-tertiary")) || AMBER;
        default:
          return (read && read("--md-primary")) || GREEN;
      }
    }
    ...
    renderContent(alert) {
      ...
      this.contentEl.innerHTML =
        `<span style="font-family:${FONT_MONO};font-size:10px;font-weight:700;letter-spacing:1.4px;text-transform:uppercase;color:${MUTED};">NUC // TERMINAL</span>` +
        `<div style="display:flex;align-items:center;gap:8px;margin-top:5px;">
          <span style="width:15px;height:15px;flex-shrink:0;color:${color};display:inline-flex;">${icon}</span>
          <span style="font-family:${FONT_MONO};font-size:11px;font-weight:700;letter-spacing:0.6px;text-transform:uppercase;color:${color};min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${escapeHtml(this.kindLabel(alert))}</span>
        </div>` +
        `<span style="font-family:${FONT_MONO};font-size:20px;font-weight:700;color:${TEXT};line-height:1.15;margin-top:5px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${nameHtml}</span>` +
        (amount ? `<span style="font-family:${FONT_MONO};font-size:14px;font-weight:700;color:${color};margin-top:4px;">${amount}</span>` : "") +
        (message ? `<span style="font-family:${FONT_MONO};font-size:12px;color:${MUTED};line-height:1.35;margin-top:4px;overflow-wrap:anywhere;word-break:break-word;">«${message}»</span>` : "");
    }
    ...
    _drawBadge(ctx, cx, cy, R, angle, color, intensity, kind) {
      ...
      // Outer ring + three radiation blades.
      ...
      ctx.lineWidth = 1;
      ctx.shadowBlur = 6 + 14 * intensity;
      ctx.beginPath();
      this._traceBlades(ctx, R);
      ctx.fill();

      // Central medallion: carve a disc out of the blades, then draw a dark
      // lead-glass core with the kind-specific symbol inside.
      ctx.save();
      ...
      ctx.lineWidth = Math.max(1, R * 0.05);
      ctx.shadowBlur = 4;
      this._drawKindSymbol(ctx, R * 0.24, kind);
      ctx.restore();

      ctx.restore();
    }

    // Trace the three blades into the current path (filled body). Spaced 120°
    // apart, each a 60° circular sector — the classic ionizing-radiation sign.
    _traceBlades(ctx, R) {
      for (let i = 0; i < 3; i++) {
        const phi = -Math.PI / 2 + i * ((Math.PI * 2) / 3);
        ctx.moveTo(0, 0);
        ctx.arc(0, 0, R, phi - Math.PI / 6, phi + Math.PI / 6);
        ctx.closePath();
      }
    }
