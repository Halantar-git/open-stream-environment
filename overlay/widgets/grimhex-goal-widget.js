...
(function (root, factory) {
  ...
  const RED = "#ff1800"; // empty sector
  const GREEN = "#00ff66"; // filled sector

  // Orbital theme fonts (Orbitron for display/mono, Rajdhani for body).
  // Single quotes keep the inline style attribute valid (no nested double quotes).
  ...
  class WidgetGrimHexGoal extends BaseWidget {
    ...
    _updateDom() {
      ...
      this.contentEl.innerHTML =
        `<span style="color:#dcebf5;font-family:${FONT_DISPLAY};font-size:13px;font-weight:600;text-transform:uppercase;letter-spacing:0.4px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${escapeHtml(goal.title || t("preview.goalTitle"))}</span>` +
        `<span style="color:#8ab4d0;font-family:${FONT_MONO};font-size:12px;white-space:nowrap;"><b style="color:${GREEN};">${formatMoney(goal.current)}</b> / ${formatMoney(goal.target)} ${escapeHtml(currencySymbol(goal.currency))}${pctStr}</span>`;
    }
    ...
    _drawSector(ctx, x, y, w, h, fill, flicker) {
      ...
      // Green fill (from the left), uniform colour.
      ctx.save();
      ctx.fillStyle = GREEN;
