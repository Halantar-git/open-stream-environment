...
- **Звуковое сопровождение розыгрыша** — музыка вращения барабана с затуханием,
  звуки победы/выбывания и настраиваемая громкость в панели управления. Звук
  победы и выбывания играет страница колеса (не оверлей): так он не дублируется,
  когда в сцене с колесом лежит и основной оверлей.
...
- **Автообновления (по кнопке)** — при старте приложение только проверяет
  ...
  «Проверить обновления» в «Настройки → Приложение»: найдя обновление, эта же
  кнопка становится «Обновить и перезапустить», а если обновления нет — рядом
  появляется «Обновлений нет». Приложение при этом ничего не
  показывает — ни баннера, ни системного уведомления: скачав обновление, оно сразу
  перезапускается.
...
- **Панели «Очередь» и «Ожидают»** — открываются вместе с панелью «DA» и стоят
  слева от неё (своих кнопок в шапке у них нет). В «Очереди» — что в эфире,
  правила и пауза: кнопка-переключатель «Пауза»/«Продолжить» (без срока — до
  повторного нажатия), «Пауза 5/15/30 мин» (со сроком, до времени),
  «Пропустить», «Очистить очередь». В «Ожидают» — счётчик и сам список ждущих
  алертов с кнопками «Показать сейчас», «Выше» и «Убрать» у каждого. Тот
...
Сохранённый пароль панель никогда не показывает: поле остаётся пустым, а
подсказка сообщает, что пароль уже сохранён — пустое поле означает «не менять».
...
- Чат: у 2D-виджета `maxMessages` по умолчанию 8; у чатов 3D-тем значение
  берётся из того же поля настроек (по умолчанию 50, максимум 200) — на хайпе
  уменьшайте окно чата.
...
Бот работает только с Twitch. Сообщения из YouTube-чата приходят в оверлей и в
окно чата, но в команды, таймеры и автомодерацию не попадают: наказание через
Helix применить к зрителю YouTube всё равно нельзя, а права модератора из чужого
чата не должны открывать команды в Twitch-чате.
...
Кнопка «DA» в шапке открывает три панели: саму DonationAlerts, слева от неё —
«Очередь алертов», а ещё левее — «Ожидают» со счётчиком и списком ждущих алертов.
Открываются и закрываются они вместе, крестик один на троих — тот же приём, что
у «Колеса» с «Участниками». Панель отвечает на вопрос «почему
...
server/
  ...
  nick-color.js           — цвет ника для чатов, которые его не отдают (YouTube и фолбэк Twitch)
...
docs/                      — заметки по разбору платформ, в сборку не входят
config/
...
Разборы платформ, которые мы рассмотрели, но пока не подключали, лежат в
`docs/` — там что доступно, чего в API нет и что мешает начать:

- [VK Video Live](docs/vk-video-live.md) — чат читать и отправлять можно,
  модерации в API нет вообще, а без аккаунта VK и зарегистрированного
  приложения нельзя даже проверить подключение: анонимного доступа у платформы
  нет.
