...
- **Панель DonationAlerts** — отдельная боковая панель (кнопка «DA» в шапке,
  ...
  живой журнал только этого сервиса. Подробнее — в разделе «DonationAlerts».
...
- **Панель «Очередь»** в шапке панели управления: что в эфире, что ждёт, и
...
Кнопка «DA» в шапке открывает боковую панель (рядом с «Логами» и «Очередью») —
она отвечает на вопрос «почему донаты не приходят», потому что искать ответ в
общих настройках или в общем терминале долго.
...
собираются и заливаются в GitHub Release установщики для Windows и Linux
(оттуда их берёт автообновление). macOS отдельной целью не собирается —
неподписанное приложение блокирует Gatekeeper, а для подписи нужны сертификаты.
...
существовать — опечатка иначе всплыла бы только в день релиза.
