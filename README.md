...
- **Клип и маркер стрима** — кнопки «Создать клип» и «Маркер стрима» на
  вкладке «Сцены» создают клип и маркер на Twitch через Helix API (нужны права
  `clips:edit` и `channel:manage:broadcast`).
- **Профили сцен** — сохранение и применение полного набора сцены: виджеты
  (раскладка), настройки сцен, заставки и активная тема (2D/3D). Профили
  хранятся в `local-db.json` и управляются на вкладке «Сцены».
