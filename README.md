...
экспортировать и импортировать в отдельный JSON-файл.

У своей темы можно выбрать **3D-стиль** — набор 3D-виджетов (Material You,
Star Citizen/Grim HEX, Nuclear, Pixel Perfect, TESO, Cobra Mk II). Тумблер «3D»
на карточке включает его: доступны все 3D-виджеты выбранного стиля, при этом
панельные 3D-виджеты (чат, цель, алерты) берут вашу палитру, а декоративные
вывески/радар/щит сохраняют фирменные цвета набора (в отличие от встроенных тем,
где 3D-вариант полностью перекрывает токены). Авто-замена 2D→3D и следование
ролей работают так же, как у встроенных тем.
